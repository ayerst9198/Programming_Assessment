...
class DisplayStats:
    ...
    def __init__(self, partner, user_scores, computer_scores):
        ...
        for question_num in range(1, 11):
            label_text = str(question_num)
            self.data_label = Label(self.scroll_frame, text=label_text,
                                    bg=odd_rows,
                                    width="15", height="3", padx=5, pady=5, wraplength=100)
            self.data_label.grid(row=question_num, column=0, sticky=W)

        # create labels for user's answers and correct answers
        for question_num in range(1, 115):
            label_text = "User's\nAnswer"
            self.data_label = Label(self.scroll_frame, text=label_text,
                                    bg=even_rows,
                                    width="15", height="3", padx=5, pady=5, wraplength=100)
            self.data_label.grid(row=question_num, column=1, sticky=W)

            label_text = "Correct\nAnswer"
            self.data_label = Label(self.scroll_frame, text=label_text,
                                    bg=odd_rows,
                                    width="15", height="3", padx=5, pady=5, wraplength=100)
            self.data_label.grid(row=question_num, column=2, sticky=W)
